#  Swiftify HowTo - StockList Demo for tvOS project
...
1. Objective-C Bridging Header file (`StockList Demo for tvOS-Bridging-Header.h`).
...
2. Precompiled Header file (`StockList Demo for tvOS-Prefix.pch`).
We are using the PCH file to conveniently import the Swift Bridging Header file (`StockList_Demo_for_tvOS-Swift.h`)

3. A Swift conversion of the original PCH file (`StockList Demo for tvOS-Prefix.swift`).
This file is always created to force generation of the Swift Bridging Header file (`StockList_Demo_for_tvOS-Swift.h`) by Xcode itself, every time the project is compiled successfully.
